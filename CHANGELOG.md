...
- **评论编辑/删除的账本语义**：编辑认领评论 → 先释放该评论原有的认领，再按新内容完整重放（把 `/claim A` 改成 `/claim B` = A 释放、B 认领；改成普通文字 = 只释放）；删除评论 → 静默释放该评论的认领并写回正文，不解析正文、不发通知
- **正文勾选双向对账（view-edit 模式）**：管理员在 issue 正文编辑清单——取消勾选已认领的文件/目录 = 手动释放，并在认领人的原认领评论上打 👎 reaction 通知；勾选未认领行 = 以编辑者为认领人入账（行为与评论认领一致；此类认领在状态块损坏自愈重建时无法从评论回放还原，详见 quick-start）
- **模板覆盖策略**：显式配置 `templatePath` 后，issue 正文每次更新以用户模板整体重建（模板为布局真相源、正文排版编辑不保留）；未显式配置时保持原位覆盖、尊重正文手写编辑

### 修复

- **树形清单中间层目录行失效**：渲染只显示目录段名（如 `ja/`），视图解析现按缩进重建完整路径（`src/ja/`），中间层目录行的勾选 / 取消从此在释放与认领两个方向都生效（此前静默失效）

### 优化

- 示例 workflow：`issue_comment` 监听 `created + edited + deleted`；新增 `issues [edited]` 触发的 view-edit job（自动过滤 bot 自身写回引发的编辑事件）
- reaction 幂等范围扩大：评论已删除（404）与重复 reaction（422）一致按幂等成功处理

## [1.2.3] - 2026-09-03

### 修复

- **sync 幂等**：看板存在已完成翻译（done）文件时，对账的变更判定把 done 条目误算为差异，导致每次运行都重复执行无意义的 body 写入；现在内容无变化时 sync 是纯 no-op（"is up to date" 早退恢复生效）
- **手工释放生产环境失效**：带链接的清单行（真实仓库默认形态）此前解析不出勾选状态，管理员取消勾选 / 删除某行对认领没有任何效果；现在链接形态与反引号形态都能对账，且视图释放会真正持久化（expire 检测到视图变更即写回，sync 的"无需更新"早退条件纳入视图变更）
- **目录行误释放**：树状清单下部分认领的子树的目录行渲染为未勾选态，不再被误读为"管理员取消勾选"而释放整棵子树——只有上一轮渲染为勾选态（子树已全认领）的目录行取消勾选才是有效释放信号

### 优化

- 重构事件模式共用的状态读取与 body 重渲染样板（`loadTrackerState` / `recomposeTrackerBody`），模板变量表只剩一份定义
- 清理未使用的 `getComment` API，`RawComment` 收敛为单一类型定义
- 新增 mode 编排级测试（内存版 GitHubApi）：sync 幂等、dry-run、claim 流程、过期释放、手工释放均有回归护栏（测试 95 → 110）
- 新增 `.gitattributes` 固定 LF 换行：Windows 上新克隆 / 新 worktree 不再因 CRLF 检出导致渲染正则与格式检查失败
...
- 清单路径显示目标语言对应文件（如 `src/en/agreement.md`），树状目录同步按目标语言路径构建；视图解析同时识别两种路径形态
