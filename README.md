...
贡献者在 issue 下评论 `/claim 文件路径` 认领，用 `/release 路径` 放弃。目录也可以一次认领：`/claim src/manual/` 会把目录下未被认领的文件全部认领，已被他人认领的自动跳过。提交的 PR 包含认领的文件，bot 自动关联并冻结超期计时；超过 `ttlDays`（默认 15 天）还没提交 PR，认领会自动释放并提醒。管理员在 issue 里直接取消勾选某一行，bot 会把它按手动释放处理。
